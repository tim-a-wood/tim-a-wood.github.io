...
- Consequence: `computeUnifiedShellWorldPlacement(asset, support)` sets **display size** to the current chamber bbox `(right−left) × (bottom−top)` in room px and positions with the same **origin** convention as placement (`localX = curLeft + originX * dw`, `localY = curTop + originY * dh`). Manifest x/y and authored display sizes are **ignored** for world placement so the outer frame always matches the polygon AABB; Phaser stretches the texture to that rectangle so the UV-locked opening tracks layout. Rejected: reintroducing polygon geometry masks to “snap” the shell; regenerating assets on every polygon tweak as the only fix.

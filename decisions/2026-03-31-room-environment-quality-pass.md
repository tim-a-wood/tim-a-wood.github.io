...
### 225. Room environment PNG generation: Gemini vs Cursor Cloud Agents (Composer) (2026-04-13)
- Status: Accepted
- Why: Founder **Google Gemini image quota exhausted**; need an alternate path for **PNG** previews (level 3) and **bespoke slot** generation without changing the whole stack.
- Consequence: **`scripts/cursor_cloud_agent_client.py`** — Cursor [Cloud Agents API](https://cursor.com/docs/cloud-agent/api/endpoints): Basic auth, `POST /v0/agents` with `model` default **`composer-2`** (`CURSOR_CLOUD_AGENT_MODEL`), poll until `FINISHED`, download **largest `.png` artifact**. **`room_environment_system.py`** — `ContextVar` + request `ai_provider` (`gemini` | `cursor_cloud`); level-3 and `_generate_image_from_references` branch to Cloud Agents when selected; missing `CURSOR_API_KEY` / `CURSOR_CLOUD_REPOSITORY` → clear errors. **`room_layout_copilot.py`** — `cursorCloudAgents` on `/api/ping`. **UI** — Step 2 segmented control (localStorage `mv-room-env-image-provider`); previews + generate-assets POST include `ai_provider`. **`.env.local.example`** — documents `CURSOR_*`, optional `MV_ROOM_ENV_IMAGE_PROVIDER`. **Rejected:** pretending Composer 2 is a drop-in Gemini image API (agents are **async, repo-scoped, slow, costly**); routing **room spec JSON** (`build_room_environment_spec` / `_gemini_json`) through Cloud Agents in this pass (still **Gemini text** unless a separate decision).

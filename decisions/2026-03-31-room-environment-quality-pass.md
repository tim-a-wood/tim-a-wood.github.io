...
- Consequence: `getOrCreatePremaskedShellTexture(scene, slotId, roomId)` draws the shell source image onto a temporary HTML Canvas, then uses **`globalCompositeOperation = 'destination-out'`** to erase the polygon interior (room-local polygon mapped to texture space via chamber AABB scaling). The resulting canvas is registered as a new Phaser texture (`env-bespoke-{slotId}-masked`). The shell sprite uses this pre-masked texture directly — no Phaser mask object at all. The polygon interior is guaranteed transparent because the Canvas 2D composite is a standard, well-tested browser API. Applied identically in play and capture for parity. Cached per slot. Rejected: `GeometryMask.invertAlpha` (WebGL stencil failure in embed); `BitmapMask` (adds another render target); centroid nudge (§201, wrong direction).

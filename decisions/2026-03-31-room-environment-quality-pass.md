...
- Why: §199–§201 all failed because the root cause was never placement — it was the **shell PNG opening itself**. Raster scan of `R1-room-shell.png` comparing actual alpha-0 regions vs the polygon-in-shell-space showed the opening was in completely wrong positions (e.g. at y=300 expected full-width opening x=[0..1079], actual x=[115..965]; at y=900 expected x=[820..1079], actual x=[0..644]). The AI-generated shell's transparent opening does not match the room polygon contract. No placement shift can fix geometry that is 400–800px wrong.
- Consequence: `ensureRoomFootprintGeometryMask(scene, roomId, roomBounds, true)` (inverted-alpha) is applied to the shell sprite in **both** play and capture modes. This clips any shell mass that overlaps the polygon interior, forcing the inner edge to track the footprint perimeter exactly. Applied in both modes for parity (§199's original goal). The centroid-nudge code (`getUnifiedShellOpeningCentroidTexture`) is removed — no longer needed because the mask enforces alignment regardless of PNG opening position. Rejected: regenerating assets on every layout (expensive, still no guarantee AI matches polygon); placement nudges (wrong direction, can't fix geometry); PNG-alpha-only approach (§199, fundamentally unreliable when AI generation doesn't match contract).

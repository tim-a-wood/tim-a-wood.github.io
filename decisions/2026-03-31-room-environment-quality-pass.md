...
### 168. Ceiling bespoke prompts must name slab contract explicitly (schema + `ceiling_band` rules)
- Status: Accepted (2026-04-07)
- Why: `ceiling_band` had **no** `component_rules` entry in `_build_bespoke_prompt`, so Gemini fell through to generic template language. The default **ceiling** schema also said **“vary arch breakup subtly”** and **“broad arch or lintel span”**, which steered the model toward multi-opening / arcade reads that still look composite after compositing.
- Consequence: Added a dedicated **`ceiling_band`** component-rules paragraph (single full-width opaque slab, no row of arched windows/grilles, no distant hall through the band). Tightened **`COMPONENT_SCHEMA_DEFAULTS["ceiling"]`** (slab/lintel language, anti-collage `negative_constraints`, mortar-only `variation_rules`). Appends **`ceiling_fields:`** to the bespoke prompt for ceiling schema. Strengthened **`ceiling_piece`** biome role and **`_retry_prompt_for_validation_errors`** for **`ceiling_piece`** and **`ceiling_band`**. Regression: `test_bespoke_prompt_ceiling_band_rejects_composite_arcade_read`.

### 169. Room footprint silhouette as Gemini reference for full-frame scenic bespoke
- Status: Accepted (2026-04-07)
- Why: Bespoke **`background_far_plate`** and **`midground_side_frame`** were conditioned only on biome template + internal geometry guide; the model had **no** explicit map of the room’s **non-rectangular polygon** at output resolution, so regenerated art still read as a generic rectangular hall. Runtime still stretches plates to the polygon AABB (see separate limitation); conditioning improves **generation** alignment before compositing.
- Consequence: **`_write_bespoke_room_silhouette_reference`** rasterizes chamber polygon + platform/door markers scaled to **`expected_size`**, prepended before template + slot guide in **`_bespoke_reference_images_for_component(..., room=...)`** for those two component types. **`_build_bespoke_prompt`** takes optional **`room_geometry`** and adds a **footprint conditioning** paragraph when `len(polygon) >= 3`. **`_validation_reference_for_component`** for scenic slots always uses **`template_path`** (not the silhouette) so **`template_family_drift`** checks stay meaningful. Regression tests: silhouette prepend + prompt clause + empty-polygon guard.
- **Amended by §227 (2026-04-13):** **`background_far_plate`** no longer prepends the silhouette image to the Gemini reference stack (guide-only + text footprint), because the banded silhouette map was teaching a second stone rim alongside **`room_shell_foreground`**. **`midground_side_frame`** still uses **`[silhouette, template, guide]`** when the polygon is valid.

### 170. Opt-in unified chamber shell (`room_shell_foreground`) + deterministic walkable punchout
- Status: Accepted (2026-04-07)
- Why: Modular **`ceiling_band`**, **`wall_module_*`**, **`wall_base_trim_*`**, and **`main_floor_*`** slots invite inconsistent scale and seam alignment; a **single** chamber-sized foreground pass (walls + ceiling + floor rim together) matches how the game already thinks about the **polygon AABB**, while the **walkable interior hole** must stay **engine-authoritative** (not model-hallucinated).
- Consequence: Env **`MV_ROOM_SHELL_FOREGROUND=1`** (see `.env.local.example`). **v3** `build_generation_plan` and **v2** `_room_component_plan` add **`room_shell_foreground`** mapped to biome **`foreground_frame`**, target size = chamber bbox from **`room.polygon`**, placement bottom-center at chamber floor; **omit** legacy shell strips and **omit** **`main_floor_top` / `main_floor_face`** when the unified slot is planned. If the polygon has fewer than three points or the biome lacks **`foreground_frame`**, planners **fall back** to legacy slots. **Generation:** Gemini path mirrors **`foreground_frame`** fitting; refs use footprint silhouette + midground-style guide; pre-validation uses **opaque** transparency mode; **`_apply_walkable_interior_punchout`** clears interior alpha using the same **`pad=24`** mapping as the silhouette; **`_validate_room_shell_after_punchout`** guards residual opacity inside the polygon. **Runtime (`index.html`):** one sprite when manifest includes the slot; skip bespoke ceiling decor, skip primary-floor bespoke merge, skip modular wall shell. Regression: planner test + punchout unit test + template map assertions.

### 170b. Room shell visual thickness (silhouette, prompts, punch inset, Python runtime review)
- Status: Accepted (2026-04-07)
- Why: Models often **copied the footprint schematic’s bright cyan outline** into the final PNG as a hairline “frame”; punchout then left only that rim, failing texture/mass expectations.
- Consequence: **`_write_bespoke_room_silhouette_reference`** uses a **neutral stone-gray** outline (not accent cyan) and a slightly thicker stroke. **`room_shell_foreground`** `component_rules` and footprint prompt copy explicitly forbid HUD-like strokes and require **heavy masonry bands** with approximate width/height fractions. Optional **`MV_ROOM_SHELL_PUNCH_INSET_PX`** (0–48, default 0) shrinks the transparent hole slightly inward via **`_inset_polygon_vertices_toward_centroid`** for a thicker surviving rim when LD accepts the tradeoff. **`_composite_runtime_review_image`** includes **`room_shell_foreground`**, scopes it like other chamber-sized layers, and sorts **`slot_group` `foreground`** between **midground** and **floor** so the shell is not drawn last. **`_validate_room_shell_after_punchout`** adds **`shell_rim_mass_low`** when **`_opaque_pixel_fraction`** is below **~1.8%** so hairline / outline-only outputs fail and trigger a targeted retry prompt.

### 170c. Anti-cyan rim + border/center contrast (biome + bespoke prompts)
- Status: Accepted (2026-04-07)
...
### 178. Shell-contract hardening: 3-region contract map, distinct guide, and material-only reference
- Status: Accepted (2026-04-09)
- Why: Live `test-40b4b333/R1` evidence showed the shell refs were still too semantically weak. Even after flattening the guide, the generator kept producing a frame-inside-a-frame raw shell with broad overreach outside the allowed shell band. Founder direction was to fix the contract upstream rather than clip or loosen validation. LD/QA review also confirmed the shell contract must explicitly distinguish the required shell band, required clear opening, and forbidden outer margin, and that the shell should stop learning from a full-scene preview.
- Consequence: `room_shell_foreground` now uses a polygon-derived 3-region contract map as reference #1, a distinct structural shell guide as reference #2, and a composition-free material-only reference as reference #3. Prompt and retry wording for shell topology are generalized away from masonry-specific nouns and now explicitly call out one shell ring, one clear opening, forbidden outer margin, and no outer/nested frame. Validation remains unchanged, including `shell_silhouette_overreach`. Rejected: deterministic clamp/postprocess of overreach as the primary fix; that would hide a bad generation instead of improving the generation contract.

### 179. Shell-contract ref cleanup: single-tone guide and safe low-impact material reference
- Status: Accepted (2026-04-09)
- Why: Founder review rejected the first contract-hardening ref outputs on visual quality grounds. The structural guide still used three grey tones across the shell band, and the first material-only builder produced scene-like ledges/panel rhythm. A second attempt at material extraction produced a visible checkerboard collage with repeated bright fragments, which was also rejected.
- Consequence: The shell guide is now a single neutral shell-band tone over transparency, removing extra top/side/foot composition cues while staying distinct from the 3-region contract map. The material-only reference was deliberately reduced to a safe, low-impact dark material field with no opening, frame, ledge, panel rhythm, or alternate geometry cue. Accepted tradeoff: the material reference is now weak but harmless; geometry authority remains in the contract map and structural guide. Validation thresholds remain unchanged.

### 180. Unified shell inner edge must equal the room perimeter; inward-rim defaults are rejected
- Status: Accepted (2026-04-10)
- Why: Founder review of runtime screenshots showed the player standing visually inside the shell border, proving the current shell contract was still teaching and preserving shell mass inside the playable room. The root causes were: the shell band mask was built as an inward ring from the polygon, the default punchout inset (`MV_ROOM_SHELL_PUNCH_INSET_PX=20`) intentionally left shell rim inside the room, and the runtime still carried centered-mask / interior-collider workarounds for shell intrusion.
- Consequence: `_room_shell_silhouette_band_mask(...)` now builds the shell band outside the chamber opening instead of inside it, restoring the room perimeter as the shell's inner edge. The default shell punch inset returns to `0`, so shell thickness is no longer preserved by moving the clear opening inward. The old runtime workaround path that re-masked the unified shell with a centered polygon stroke and added interior side colliders is no longer used for the unified shell path. Rejected: keeping an inward rim by default and compensating with runtime colliders; that preserves the wrong contract and keeps the player visually inside shell mass.

### 181. Room-shell generation must preserve fixed canvas geometry and fail explicit dark-shell cases
- Status: Accepted (2026-04-10)
- Why: The room shell was still sharing the `foreground_frame` fit helper, which trims edge-connected background and rectangularizes output. In parallel, the material-only shell reference was biased toward low-information dark patches, and the retry path still referenced `shell_tone_too_dark` even though the validator no longer emitted that code consistently.
- Consequence: `room_shell_foreground` now uses a shell-specific fit path that preserves exact canvas dimensions and does not trim to a rectangular bounding box. `_room_shell_material_reference(...)` now selects higher-information material patches and lifts the final reference to a readable mid-dark floor instead of feeding the model a muddy near-black wash. `_validate_room_shell_after_punchout(...)` once again emits `shell_tone_too_dark` explicitly and hard-fails `shell_silhouette_underfill` when the shell band is not sufficiently occupied. Rejected: softening validation or relying on runtime masking to hide a dark or geometrically wrong shell.

### 182. Runtime shell masking now cuts out the room opening instead of centering a shell stroke on the perimeter
- Status: Accepted (2026-04-10)
- Why: The remaining live shell failures came from two geometry mismatches: the contract map still treated the shell as an inward ring, and runtime masking still used a centered thick polygon stroke. That combination let shell mass survive inside the opening and obscured the real geometry bug.
- Consequence: `_write_room_shell_contract_map_reference(...)` now keeps the full polygon as the clear opening while `_room_shell_silhouette_band_mask(...)` remains the authoritative outer shell band outside that opening. Runtime `applyUnifiedShellFootprintMask(...)` now fills the polygon and inverts the geometry mask so the shell sprite is cut away inside the room instead of preserved by a centered stroke. Validation and its matching retry hooks were kept intact, including `shell_tone_too_dark`. Rejected: keeping the inward-ring contract and compensating with a thicker runtime stroke or weaker overreach logic.

### 183. Obsolete structural sibling templates are forbidden in the shell debug-material path
- Status: Accepted (2026-04-10)
- Why: During corrective planning for the shell reference/material failures, one proposed fallback was to source shell debug material from legacy structural sibling templates such as `wall_piece`, `ceiling_piece`, or `primary_floor_piece`. Founder direction explicitly rejected that route because those families are obsolete for the current shell workflow and would reintroduce old contract drift under the guise of a debug fix.
- Consequence: The shell debug-material corrective pass must not use `wall_piece`, `ceiling_piece`, or `primary_floor_piece` as direct sources, fallbacks, or temporary substitutions. Any implementation brief, QA gate, or future corrective work that routes shell debug material through those template families is considered a regression and should fail review immediately. Accepted alternatives are limited to shell-specific sources only, such as sanitized approved-preview sampling or a deterministic shell-only debug texture created specifically for shell validation.

### 184. Live shell refs are limited to contract, guide, and deterministic shell-safe material
- Status: Accepted (2026-04-10)
- Why: `room_shell_foreground` review was being polluted by stale `_refs` artifacts (`silhouette`, `preview-tone`) and by the possibility of raw preview/template fallback re-entering the active shell reference stack. That made visual QA untrustworthy and reintroduced non-shell semantics into a material-only slot.
- Consequence: The active shell reference set is now only `contract`, `guide`, and deterministic `material`. Stale `silhouette` / `preview-tone` shell artifacts are cleaned out of the active `_refs` folder, and raw preview/template fallback is no longer allowed in the shell ref list. QA should treat any reappearance of those stale files or fallback refs as a regression.

### 185. Shell geometry uses shell-band margin instead of the old fixed 24px shell pad
- Status: Accepted (2026-04-10)
- Why: Shell refs, punchout, and post-punchout validation were all sharing a fixed `24px` shell pad even though the intended shell band width was size-dependent. That mismatch caused visible wall-thickness inconsistency at the shell edge and taught malformed shell guides.
- Consequence: Shell-specific geometry now derives its reserved margin from the shell-band size function rather than the old fixed `24px` pad. The same shell-band margin is used consistently across shell contract/guide generation, punchout, and post-punchout validation. Future shell-geometry changes must update those paths together or they will break the contract.

### 186. The pre-punchout shell corner-gap validator is active again
- Status: Accepted (2026-04-10)
- Why: The pre-punchout shell corner-gap check had a dead comparison (`< 0.0`), which made `shell_corner_gap_pre` unreachable even for obviously missing corners. That weakened the shell QA gate and hid broken candidates until later stages.
- Consequence: The pre-punchout corner-gap guardrail now uses a real threshold again. QA should keep one regression proving `shell_corner_gap_pre` is reachable for a visibly missing corner and one control proving valid dark corners still pass.

### 187. Concave-shell corner-gap validation must follow the authoritative shell band, not all four image corners
- Status: Accepted (2026-04-10)
- Why: After the shell contract moved to an outer-band model for non-rectangular rooms, live `Room AI Helpfulness QA / R1` still failed consistently on `shell_corner_gap_pre` even when the saved raw shell matched the intended concave opening. QA and visual inspection showed the blocker was a contract-validator mismatch: the corner-gap checks were still hard-checking all four image corners, but the authoritative shell contract for this L-shaped room intentionally leaves some image corners as transparent forbidden margin.
- Consequence: The pre- and post-punchout corner-gap validators are now geometry-aware. They derive which corner boxes are actually required from the authoritative shell-band mask and only hard-fail `shell_corner_gap_pre` / `shell_corner_gap_post` where the contract says shell mass must occupy that corner. This keeps the hard corner-gap gates for required shell corners without falsely failing concave rooms whose outer margin legitimately leaves one or more image corners empty. Rejected: lowering the threshold, disabling the corner-gap codes, or loosening silhouette validation to push concave runs through.

### 188. Shell material reference now follows the shell band contract, and shell retries accumulate multiple dominant failure instructions
- Status: Accepted (2026-04-10)
- Why: After the geometry-aware corner-gap fix, the live `R1` shell no longer failed on the false precheck. The real remaining blockers were `shell_band_tone_drift`, `shell_corner_join_seam_read`, and `shell_silhouette_overreach`. Developer and visual review showed two upstream causes: the material-only shell reference was still a full-frame directional masonry swatch that encouraged overreach and band-to-band drift, and the shell retry prompt returned after the first matching error, so retries kept spending turns on darkness while ignoring the dominant remaining blockers.
- Consequence: `_room_shell_material_reference(...)` now honors the shell contract when geometry is available: shell material appears only in the shell band, the opening remains the clear-opening neutral, and the forbidden outer margin stays transparent. The debug texture itself is less directionally biased so it teaches a more uniform shell family across top, sides, and floor bands. `_retry_prompt_for_validation_errors(...)` for `room_shell_foreground` now accumulates multiple shell-failure instructions in one retry prompt, with overreach, corner joins, and tone drift all able to land together instead of only the first matching error. Rejected: clipping generated shell output as the first response; the approved direction remains to improve the generation contract upstream before deterministic hiding.

### 189. Shell postprocess must clamp alpha to the authoritative shell band, not only punch out the opening
- Status: Accepted (2026-04-10)
- Why: Live `Room AI Helpfulness QA / R1` still failed consistently after the shell-contract and retry-prompt fixes, even though the saved raw shell visibly followed the stepped/L-shaped opening more closely. Direct local probing showed the root cause was a contract mismatch inside postprocess: `_apply_walkable_interior_punchout(...)` only cleared the walkable interior polygon, so opaque near-black matte survived in the forbidden outer margin. The post-punchout validator, however, judges shell legality against `_room_shell_silhouette_band_mask(...)`, so the surviving matte was being counted as shell mass and triggering `shell_silhouette_overreach`, plus secondary tone/corner failures. When the same raw shell was locally clamped to the authoritative band mask and then run through the existing tone/detail/corner-soften steps, the validator passed.
- Consequence: `_apply_walkable_interior_punchout(...)` now multiplies alpha by the authoritative shell band mask instead of only subtracting the interior polygon. That forces both the clear opening and the forbidden outer margin back to transparent alpha before normalization, detail recovery, and corner-shadow softening. Added QA coverage proves postprocess clears forbidden outer-margin opacity for a concave/L-shaped room instead of leaving opaque black filler behind. Rejected: loosening `shell_silhouette_overreach` or treating the black matte as visually acceptable “empty” space; the saved alpha must match the shell contract.

### 190. Background shell-definition salvage must not paint synthetic guide lines into the scenic plate
- Status: Accepted (2026-04-11)
- Why: After the shell itself was corrected, runtime review still showed hard cutoff lines through the opening. Direct visual inspection of the exact saved artifacts showed the same dark vertical bars and repeated arch-like caps already baked into `R1-background.png`, while the standalone shell asset remained clean. Code tracing narrowed the source to `_restore_background_shell_definition(...)`, which was drawing synthetic narrow vertical bands and arch arcs back onto `background_far_plate` during salvage. Those overlays were intended to strengthen enclosure read, but in practice they taught and preserved debug-looking cutoff lines in the final scenic plate.
- Consequence: `_restore_background_shell_definition(...)` now keeps only the softer template-guided shell-definition blend and no longer paints synthetic column/arch overlays into the saved background plate. Added regression coverage proves the helper does not add guide-line artifacts when no template source is provided. Rejected: masking the lines only in runtime review, since that would leave the saved background asset malformed and make QA evidence misleading.

### 191. Runtime scenic layers must be clipped to the room opening, not left visible in shell-forbidden outer margins
- Status: Accepted (2026-04-11)
- Why: After removing the synthetic blocker lines from `background_far_plate`, the runtime review still made the border look broken. Exact visual inspection showed the shell opening was coherent, but background detail was still visible above the top band and in other shell-forbidden outer-margin zones where the shell is intentionally transparent. That made the border read like a damaged or incomplete frame even though the shell itself was shaped correctly.
- Consequence: browser runtime background, midground, and background-feather layers now share the same room-opening polygon mask used for scenic visibility, while the shell continues to use the inverted outer-shell mask. The Python runtime-review compositor now applies the same room-opening alpha mask to `background_far_plate` and `midground_side_frame` before compositing, so saved review artifacts match the live runtime contract. Added QA coverage proves scenic layers are clipped to the non-rectangular room opening instead of filling shell-forbidden outer-margin zones. Rejected: re-rolling the shell again or treating the broken-border read as a shell-generation issue.

### 192. Composite-fallback runtime review must not reintroduce fake shell breakage through helper slabs or unsliced scenic overlays
- Status: Accepted (2026-04-11)
- Why: After scenic layers were clipped to the room opening, the saved `runtime-review.png` still looked like a broken shell when browser capture fell back to the Python compositor. Direct visual inspection showed two fallback-only mismatches: the compositor was still drawing deterministic helper masonry under a valid unified shell, and its scenic layers did not fully match the browser runtime’s softer review read. Those fallback artifacts made the shell border look damaged even when the shell asset itself was fine.
- Consequence: the fallback runtime-review compositor now suppresses helper masonry slabs when `room_shell_foreground` is present, keeping shell-forbidden outer margins dark instead of filling them with fake support structure. Additional QA coverage protects both the room-opening clip and the “no helper slab under unified shell” behavior. Rejected: treating the broken-border read as a fresh shell-generation problem; the failure was in review composition parity.

### 193. Midground is temporarily disabled in runtime and fallback review while isolating shell-border breakage
- Status: Accepted (2026-04-11)
- Why: Founder manual workbench runs still showed a broken-looking border even after the shell, scenic opening clip, and fallback helper-slab fixes. To isolate whether the midground layer was still interfering with the shell read, the safest short-term move was to remove midground from both the browser runtime composition and the Python fallback review path.
- Consequence: runtime composition now skips bespoke midground placement behind a temporary `DISABLE_RUNTIME_MIDGROUND` / `RUNTIME_REVIEW_DISABLE_MIDGROUND` switch, and QA includes a regression proving fallback review does not composite midground while the switch is enabled. This is an isolation switch, not a product-end-state decision; midground should only come back after the shell-border issue is understood and verified visually.

### 194. Unified-shell runtime rooms must not add extra procedural shell overlays or heavy feather fades on top of bespoke shell/background art
- Status: Accepted (2026-04-11)
- Why: Founder manual workbench runs still showed a broken-looking room preview even when the standalone shell PNG and background PNG both looked acceptable. Code review of the browser runtime path showed two extra runtime-only layers still drawing over bespoke rooms: procedural `addShellOverlays()` graphics based on `DUNGEON_WALL_RECTS`, and high-alpha background feather fades. Those layers do not belong to the generated shell/background artifacts themselves and can create false border/cutoff-line reads inside the opening.
- Consequence: unified-shell runtime rooms now suppress procedural shell overlays, and bespoke/unified-shell rooms suppress the heavy background feather fades that were previously used as a generic scenic post-process. This is intentionally scoped to the browser/runtime composition path so we remove runtime-only contamination without reopening shell or background generation contracts. Rejected: continuing to tune shell/background generation while extra runtime overlays remained free to distort the composed room.

### 195. Background shell-definition salvage is fully disabled; scenic plates must not regrow deterministic side bands or top caps
- Status: Accepted (2026-04-11)
- Why: Founder evidence from the Generated Images panel showed the broken room read was still present even when runtime-only midground and overlay cleanup had landed. Direct inspection of the exact saved `R1-background.png` proved the vertical dark bars were already baked into the background plate itself. The remaining source matched `_restore_background_shell_definition(...)`, which still composited a hard-coded left/right band plus top-cap mask back into `background_far_plate` whenever template-guided recovery ran.
- Consequence: `_restore_background_shell_definition(...)` is now a no-op. Background salvage may still suppress hot center lanes, but it must not inject deterministic shell-like framing back into the scenic plate. QA now includes a regression proving the helper does not change the image even when a template is provided. Rejected: keeping a “softer” version of the shell-definition restore; that still teaches or reintroduces fake structural side bands into background art.

### 196. `background_far_plate` bespoke prompt must describe the attached footprint silhouette (contract parity with reference stack)
- Status: Accepted (2026-04-11)
- Why: `_bespoke_reference_images_for_component` already prepends `_write_bespoke_room_silhouette_reference(...)` for `background_far_plate` when room geometry has a polygon, but `_build_bespoke_prompt` deliberately omitted any “Room footprint conditioning” clause for that component type. That told the model a footprint map was present in the component rules while the structured prompt body did not reinforce how to read the first reference image, encouraging rectangular-nave drift and weak alignment with L-shaped voids.
- Consequence: When `background_far_plate` is built with a valid `polygon`, the bespoke prompt now includes a background-specific footprint clause (void vs band, no nave substitution, explicit rejection of tall black vertical slats in the void) plus chamber bounds, without reusing the shell-only “border masonry occupancy” sentence reserved for structural slots. QA renames/extends the prompt test to require this clause. Rejected: reusing the exact midground/shell silhouette wording for background, which could mis-train the model about where to place far-depth mass vs interior void.

### 197. Runtime review: unified shell must sort above platform ledges in capture mode
- Status: Accepted (2026-04-11)
- Why: Founder QA reported the runtime review frame looking **incomplete**—shell border segments appeared **cut off** though the shell PNG itself was fine. A CV-style raster pass (alpha/luminance structure on `runtime-review.png`, `R1-room-shell.png`) could not recover layer order from the flattened screenshot, but code review of Phaser `setDepth` ordering showed `addRoomBespokeUnifiedShellForegroundDecor` used depth **0.1** in `RUNTIME_REVIEW_CAPTURE_MODE` while bespoke **platform ledges** and similar accents use **0.11**. Higher depth draws on top, so ledges were **occluding** the unified shell in capture frames, mimicking a bad mask or broken shell.
...
### 200. Unified shell placement must track the runtime polygon AABB, not stale manifest x/y/size
- Status: Accepted (2026-04-11)
- Why: After §199, founder still saw the shell **inner edge** drift from the room footprint in playtest: `addRoomBespokeUnifiedShellForegroundDecor` trusted bespoke `placement.x` / `placement.y` and `display_width` / `display_height` from the manifest. Those values are correct when the layout matches generation-time `local_geometry`, but any **layout drift**, hand-edited polygon, or embed/hash layout that does not rewrite bespoke placement leaves the shell anchored to **old** coordinates while background/midground and physics use **current** `getRoomPolygonBounds`—so the opening no longer hugs the perimeter.
- Consequence: `computeUnifiedShellWorldPlacement(scene, asset, support)` sets **display size** to the current chamber bbox `(right−left) × (bottom−top)` in room px and positions with the same **origin** convention as placement, then §201 may nudge using the transparent-opening centroid. Manifest x/y and authored display sizes are **ignored** for the pre-nudge anchor. Rejected: reintroducing polygon geometry masks to “snap” the shell; regenerating assets on every polygon tweak as the only fix. **Placement mechanics superseded by §213** (first arg is `roomId`; top-left AABB anchor, no manifest x/y).

### 201. Unified shell: nudge placement by transparent-opening centroid vs image center (asymmetric L-shell frames)
- Status: **Superseded by §202** (2026-04-12)
- Why: §200 aligned the **image** outer bbox to the chamber AABB, but founder evidence still showed the **walkable hole** shifted. Raster analysis showed alpha0 centroid offset from image center due to asymmetric pier mass. The approach was to nudge the sprite by the centroid delta.
- Consequence: The nudge shifted the sprite ~31px right and ~8px up, which **moved the opening away from the polygon perimeter** instead of toward it. Root cause investigation (§202) revealed the PNG opening geometry was fundamentally wrong — the centroid approach was compensating in the wrong direction.

### 202. Unified shell: inverted-alpha polygon mask in both play and capture (PNG opening unreliable)
- Status: **Superseded by §202b** (2026-04-12)
- Why: Raster scan proved shell PNG opening was 400–800px wrong. Inverted-alpha GeometryMask applied in both modes.
- Consequence: Mask worked in headless capture (runtime-review.png showed correct L-shaped opening) but **did not take effect in live playtest embed** — likely a Phaser 3.60 WebGL stencil/invertAlpha rendering path issue where the inverted geometry mask silently fails to clip in the live render loop.

### 202b. Unified shell: canvas-premasked texture (destination-out polygon punch)
- Status: Accepted (2026-04-12)
- Why: §202's `GeometryMask.invertAlpha = true` did not clip shell mass in the live playtest WebGL path (founder confirmed same visual after deploy). The headless-capture screenshot looked correct because it may use Canvas2D or a different stencil init path. A rendering-pipeline-independent approach is needed.
- Consequence: `getOrCreatePremaskedShellTexture(scene, slotId, roomId)` draws the shell source image onto a temporary HTML Canvas, then uses **`globalCompositeOperation = 'destination-out'`** to erase the polygon interior (room-local polygon mapped to texture space via chamber AABB scaling). The resulting canvas is registered as a new Phaser texture keyed **`env-bespoke-{slotId}-masked-{footprintHash}`** (§207: hash of polygon + chamber AABB so layout edits do not reuse a stale punch; earlier text `…-masked` without suffix is superseded). The shell sprite uses this pre-masked texture directly — no Phaser mask object at all. The polygon interior is guaranteed transparent because the Canvas 2D composite is a standard, well-tested browser API. Applied identically in play and capture for parity. Rejected: `GeometryMask.invertAlpha` (WebGL stencil failure in embed); `BitmapMask` (adds another render target); centroid nudge (§201, wrong direction).

### 203. Room editor: Layout canvas and Environment overlay use the same aspect model (2026-04-12)
- Status: Accepted
- Why: Audits found the Layout canvas used independent `scaleX`/`scaleY` while the Environment “Layout overlay” SVG uses uniform room-pixel scaling, so the same polygon could look geometrically different; `roomWizardOverlayBounds` could also derive `viewBox` from `max(vertex)` when assembly `size` was missing, disagreeing with full `room.size` for inset footprints.
- Consequence: `roomScale()` uses `Math.min(scaleX, scaleY)` with letterboxing. `roomWizardOverlayBounds` prefers `assembly_plan.overlay_geometry.size`, then `room_semantics.room_size`, then the wizard room’s `size`, then polygon span, then legacy fallbacks. Overlay copy distinguishes cyan footprint vs gold structural slots; thinner SVG strokes for footprint/shell lines. **Out of scope here:** unified-shell AABB vs concave polygon (planner/runtime contract per §200–§202b).

### 204. Minimal bespoke compositor is opt-in until visual gate passes (2026-04-12)
- Status: Accepted
...
### 207. Shell fixes: premask cache keys footprint; prompt rejects black collage gutters (2026-04-12)
- Status: Accepted
- Why: Founder reported shell read still broken after policy docs; investigation showed (1) `getOrCreatePremaskedShellTexture` cached `env-bespoke-{slotId}-masked` with **no** polygon fingerprint, so layout/polygon edits could reuse a **stale** Canvas punch; (2) Gemini still produced **black separator bands** inside the shell band despite contract maps — tighten **prompt language only** (§206: no new strict validators).
- Consequence: `index.html` — `hashRoomFootprintForShellPremask(roomId)`; premask texture key is `…-masked-{hash}`. `scripts/room_environment_system.py` — `room_shell_foreground` bespoke prompt adds anti-collage / anti–void-gutter clauses (continuous masonry ring, no flat black stripes through the band). **Rejected:** deterministic raster heal of black bands (§205); new auto-reject thresholds for collage (§206).

### 208. Background plate must not duplicate unified shell; runtime vaultHeight matches shell when unified shell (2026-04-12)
- Status: Accepted
- Why: Founder evidence showed `background_far_plate` prompts literally asked for a “far-depth **hall shell**,” teaching a **second** masonry frame inside the scenic plate while `room_shell_foreground` already provides the rim. Separately, `getRoomBespokeCeilingCapHeightForRuntime` still reserved top band height from `ceiling_band` assets even when unified shell was active — `addRoomBespokeCeilingDecor` is already skipped for unified-shell rooms, so background/midground were scaled to **`vaultHeight` &lt; chamber** while the unified shell used **full chamber** height → **misaligned** stacks.
- Consequence: `scripts/room_environment_system.py` — `background_far_plate` rules and footprint clause: **interior depth only**, explicit **no duplicate perimeter frame** / no second shell; retry for `background_shell_definition_low` deepens **void** depth, not outer rim. `index.html` — `getRoomBespokeCeilingCapHeightForRuntime` returns **0** when `roomHasBespokeUnifiedShellForeground(roomId)` so background uses **full `chamberHeight`** like the shell. **Rejected:** keeping “hall shell” language on the background plate; keeping vault crop when unified shell owns the top rim.

### 209. Full pipeline follow-up: background schema/guide and runtime placement must stop teaching or restaging a second shell (2026-04-12)
- Status: Accepted
- Why: Founder re-ran R1 and the saved artifacts still showed a large shell-like frame in `R1-background.png`, plus the runtime review continued to compound that frame against the real unified shell. Follow-up audit found three remaining cross-stack contradictions: 1. background schema defaults still described an **enclosing hall shell**, 2. biome/template guide generation for `background_far_plate` still drew a shell-like side-frame/portal reference, and 3. runtime bespoke background placement still stretched the plate to the footprint chamber box instead of honoring the authored manifest placement/display size before masking.
- Consequence: `scripts/room_environment_system.py` background schema defaults now describe **interior depth only** and explicitly reject a duplicate perimeter shell; biome `background_plate` template prompt removes room-shell rhythm language; `_room_component_plan` / v3 planner now use `border_treatment: interior_depth` for `background_far_plate`; `_structural_slot_reference_guide(..., "background_far_plate")` now draws a calm depth guide instead of a framed shell diagram. `index.html` now uses bespoke background manifest placement/display size for `background_far_plate` before applying the polygon mask, and footprint masks for background/midground use `ROOM_IDS[roomId]` world bounds. Existing bad PNGs remain bad until regenerated; this change fixes the contract for future generation/runtime, not by deterministic repaint. Rejected: adding another deterministic postprocess to erase shell read from already-saved background PNGs (§205).

### 210. Missed dependency: v3 planner and editor-side background schema must stop emitting the old shell contract (2026-04-12)
- Status: Accepted
- Why: After §209 landed, founder regenerated R1 and the saved `environment_manifest.json` still showed `background_far_plate.border_treatment: "full_frame"`, proving one planner path still emitted the old background contract. Audit also found `room-wizard-environment.js` still described background as an **enclosing hall shell**, which could keep UI/state surfaces aligned to stale semantics even when Python prompt code changed.
- Consequence: `scripts/room_environment_v3.py` now emits `border_treatment="interior_depth"` for `background_far_plate`, and `room-wizard-environment.js` background defaults now match the new contract (`far-depth architectural stone interior`, `interior depth only`, no duplicate perimeter shell). This closes the remaining stale source that was reintroducing the old shell-like framing after regeneration. Rejected: assuming the Python prompt fix alone covered all active planner/editor sources.

### 211. Background bespoke generation must not inherit shell framing from stale saved schema fields or the biome `background_plate` reference image (2026-04-12)
- Status: Accepted
- Why: Founder regenerated `R1-background.png` after §210 and the asset still painted a clear second shell. Live prompt audit showed two remaining leaks outside the planner manifest: 1. saved `room.environment.spec.component_schemas.background` values from older projects still contained phrases like `far-depth architectural stone shell`, `enclosing hall shell`, and `must read as enclosing shell`, and those values were being injected directly into the bespoke prompt; 2. `background_far_plate` still received the biome `art_direction_biomes/.../background_plate.png` as a reference image, and that image itself visually read like a framed chamber, so Gemini was being asked to preserve the very composition we wanted removed.
- Consequence: `scripts/room_environment_system.py` now sanitizes stale saved background-schema fields back to the current **interior-depth-only** contract during normalization, strengthens the background composition contract so references are treated as material/depth guidance rather than a composition to preserve, and changes `background_far_plate` bespoke references to use only the room silhouette plus generated depth guide (no live biome `background_plate` image in the prompt). Validation reference selection for `background_far_plate` now follows the generated refs instead of the framed biome template, so prompt-building and regression checks stay aligned with the new contract. Rejected: continuing to trust room-saved schema text blindly; continuing to feed a shell-like biome template into background bespoke generation while asking Gemini not to paint a shell.

### 212. Unified shell: Canvas footprint premask is opt-in; default trusts PNG alpha (2026-04-12)
- Status: Accepted
- Why: Founder compared isolated `R1-room-shell.png` (complete stepped ring) to `runtime-review.png` and saw a **cut-up** frame with black voids. The §202b `destination-out` punch maps the **layout polygon** onto the shell texture; on **concave / L-shaped** footprints, minor mis-registration between the contract opening and the premask polygon **erodes real masonry** (destination-out removes pixels that are still visually part of the stone ring). That reads as floating fragments and “shell not fully shown,” not a layering bug.
- Consequence: `index.html` — `UNIFIED_SHELL_FOOTPRINT_PREMASK` via `?shellFootprintPremask=1` only; **default** unified shell uses **`env-bespoke-{slotId}`** (raw PNG). `MINIMAL_BESPOKE_COMPOSITOR` still forces raw texture (no premask). Depth stays **0.16** (above floor cap / ledge, below doors). **Amends §206** item 1 for this narrow point: product QA/playtest no longer defaults to premask; use `?shellFootprintPremask=1` when polygon-perfect hole alignment matters more than preserving the full authored ring. Rejected: keeping premask as default while R1 L-shell visibly loses ring mass; deterministic “grow polygon” hacks instead of opt-in premask.

### 213. Unified shell placement: top-left locked to polygon AABB (not full room/viewport rect)
- Status: Accepted (2026-04-12)
- Why: Playtest showed the shell **inner edge** misaligned with the **footprint polygon** drawn in the layout editor: placement still effectively referenced **room canvas** extents or manifest anchor math instead of the same **chamber box** the grid/perimeter uses. The shell contract maps texture `(0,0)` to the chamber top-left; world position must be `roomBounds.start + polygonAABB.left/top` with `setOrigin(0,0)` and display size from that AABB span.
- Consequence: `computeUnifiedShellWorldPlacement(roomId, asset, support)` reads `getRoomPolygonBounds(roomId)`, rounds left/top/right/bottom, sets `dw`/`dh` from the span (manifest display size only if within 3px), returns **`ox: 0, oy: 0, localX: curLeft, localY: curTop`** — manifest `placement.x` / `placement.y` are not used for unified shell. `addRoomBespokeUnifiedShellForegroundDecor` passes `roomId`; unified shell sprite calls `setRoundPixels(true)` when available. **Amends §200** consequence text (origin-based localX math and scene arg are obsolete). Rejected: anchoring the shell to the full room rectangle or trusting manifest position when it can desync from the live polygon.
- **Amended by §214:** AABB-only **display size** is insufficient when the PNG alpha **hole** does not span the full frame (see §214).

### 214. Unified shell raw PNG: scale and offset from measured interior hole bbox (2026-04-12)
- Status: Accepted (2026-04-12)
- Why: Founder playtest after §213 still showed inner masonry vs footprint grid wrong. Raster check on `R1-room-shell.png` (1080×960): fully transparent pixels’ AABB is **966×960**, not 1080×960 — the right ~114px of the file is opaque stone. `setDisplaySize(chamberW, chamberH)` maps the **entire** bitmap to the chamber box, so the **hole** only covers 966/1080 of the chamber width — inner vertical stone sits too far **inboard** on the right and can overlap the walkable grid / read as shifted vs the polygon AABB.
- Consequence: `measureShellInteriorHoleBBoxFromTexture(scene, baseTexKey)` (Canvas `getImageData`, alpha &lt; 8, cached) supplies hole `(x,y,w,h)` in source pixels. For **raw** unified shell (no Canvas premask): `dw = sw * chamberW / hole.w`, `dh = sh * chamberH / hole.h`, `localX = chamberLeft - hole.x * (dw/sw)`, `localY = chamberTop - hole.y * (dh/sh)`, rounded. Fallback when measurement fails: prior AABB + manifest display epsilon. **§202b premask path unchanged:** texture space stays `w × h` mapped 1:1 to chamber AABB for the polygon punch; sprite uses `dw = chamberW`, `dh = chamberH`, top-left chamber. Rejected: relying on manifest `display_width` alone to fix hole geometry; regenerating every shell to force hole width = frame width as the only runtime fix.
- **Amended by §215:** hole bbox must be the **largest 4-connected** low-alpha component, not the envelope of all transparent pixels (see §215).

### 215. Unified shell hole measurement: largest connected transparent component (2026-04-12)
- Status: Accepted (2026-04-12)
- Why: After §214, founder still saw the frame shifted **up and left** vs the footprint. Raster analysis of `R1-room-shell.png`: transparent pixels are **not** one connected region. There is a partial-width transparent band **y=0..105**, a fully **opaque** stone band **y=106..114**, then the **main** walkable void. A single min/max bbox over all `alpha < 8` pixels yields `minY = 0`, so vertical scale and `localY` treat the spurious top band as part of the hole; the main void’s top (`y=115`) then maps **below** `chamberTop` in world space and the inner bevel no longer tracks the polygon top.
- Consequence: `measureShellInteriorHoleBBoxFromTexture` flood-fills 4-neighborhood on `alpha < 8`, keeps only the **largest** component by pixel count, then returns that component’s axis-aligned bbox (for R1: approximately x=115..965, y=115..845, i.e. 851×731). Placement math in `computeUnifiedShellWorldPlacement` unchanged: scale chamber span by `hole.w`/`hole.h`, offset by `hole.x`/`hole.y`. Premask path unchanged. Rejected: hand-tuned `localX`/`localY` nudges without deriving from the raster; using only the envelope of `alpha==0` (same minY bug as full envelope when slivers share the bbox).

### 216. Baseline: R1 unified shell placement locked for regression (2026-04-12)
- Status: Accepted (2026-04-12)
- Why: Founder confirmed the connected-component hole + chamber AABB placement is correct in playtest; future edits must not silently drift hole math or golden dimensions.
- Consequence: `tests/fixtures/unified_shell_r1_placement_baseline.json` holds hole bbox, chamber AABB, and rounded `display_width` / `display_height` / room-local `local_x` / `local_y` for R1. `tests/unified_shell_hole_baseline.test.py` runs a synthetic raster test in CI and asserts the JSON when the local R1 PNG exists. `docs/baselines/unified-shell-placement-r1.md` describes the contract and optional `git tag baseline/unified-shell-r1-2026-04-12`. Rejected: relying only on string scans in `game-logic.test.js` without numeric raster regression.

### 217. Runtime review: skip margin occlusion under unified shell; background prompt = 2D orthographic (2026-04-12)
- Status: Accepted
- Why: Founder runtime/workbench still showed harsh rectangular dark bands overlapping the jagged unified shell + masked background — `_composite_runtime_review_image` was still compositing full-height left/right/bottom **occlusion** rectangles even when `has_unified_shell` already skipped helper masonry; that stacked a second “frame” read. Separately, `background_far_plate` generation still skewed toward one-point tunnel perspective; prompts and the structural depth guide needed explicit **2D side-scroller / orthographic** language and a guide that does not teach a second perimeter (horizontal bands only).
- Consequence: `scripts/room_environment_system.py` — occlusion compositing runs only when `not has_unified_shell`. `background_far_plate`: `prompt_opening`, `component_rules`, `silhouette_clause`, and `_structural_slot_reference_guide` updated per above (see §209–211 for related shell-duplication contract). **Rejected:** keeping review occlusion on unified-shell rooms; reintroducing shell-like vertical slab + center “tunnel” in the generated depth guide.

### 218. Room environment preview (level 3) + art-direction frozen concepts: explicit 2D orthographic perspective (2026-04-13)
- Status: Accepted
- Why: Bespoke `background_far_plate` already asks for side-scroller orthographic read, but **Gemini room preview images** (`_build_level3_variant_prompt`) and **project art direction concept** prompts (`_build_art_direction_concept_prompts`) still allowed strong one-point / tunnel reads; frozen concepts leak perspective into preview generations.
- Consequence: `scripts/room_environment_system.py` — level-3 preview prompt adds orthographic / anti–vanishing-corridor language, output negatives for fisheye and one-point corridor matting, and variant note 1 stresses layering not perspective tunnel; art-direction concept prompts include the same perspective clause. **Rejected:** relying on “side-view” alone without negating convergence cues.

### 219. Level-3 preview: layout conditioning de-schematic + prompt hygiene; biome kit not in reference stack (2026-04-13)
- Status: Accepted
- Why: Founder previews showed **diagram-like lines** (polygon/platform/door strokes, labels) and persistent **3D tunnel** reads. **Biome template PNGs are not attached** to `_generate_level3_image_with_gemini` — only `_render_room_layout_conditioning_image` plus up to three **frozen art-direction concept** images (`locked_concept_ids` on the matching biome pack or project `frozen_concept_ids`). Strong lines and gold accents in the conditioning plate and hero-style concepts were leaking; prompts did not forbid tracing the guide or copying concept camera.
- Consequence: `scripts/room_environment_system.py` — `_render_room_layout_conditioning_image` softened (no on-image text, muted outlines, no bright gold platform/door strokes, weaker focal glow); `_build_level3_variant_prompt` adds layout-guide hygiene, schematic-line ban, and frozen-concept “palette/mood only — do not copy perspective or layout” when `image_path` refs exist. **Rejected:** attaching biome kit tiles to preview Gemini calls (would add more framed-shell references); relying on prompt text alone without de-schematicing the conditioning raster.

### 220. Level-3 preview: chamber-aligned conditioning + numeric footprint contract (2026-04-13)
- Status: Accepted
- Why: Previews still **did not match room shape** and sometimes drifted from strict2D. Root cause: the layout guide drew the polygon with `_fit_points` (normalized to the **polygon’s own bbox**) while platforms/doors used **full room width/height**, so **L-shaped or inset chambers** were geometrically inconsistent in the first reference image; the model then invented a simpler nave. Variant-specific **hero glow** also biased composition away from the footprint.
- Consequence: `scripts/room_environment_system.py` — `_render_room_layout_conditioning_image` uses `_geometry_footprint_polygon_output_pixels` + `_chamber_point_to_output_pixel` with a single `pad` so polygon, platforms, and doors share one chamber mapping; platform span uses `len×32` layout units scaled into the guide; removed variant glow. `_build_level3_footprint_coordinate_block` injects chamber bounds, ordered polygon vertices, and platform/door anchors into the Gemini text prompt; perspective paragraph adds **vertical picture plane / layered flats** language. **Rejected:** keeping polygon-only-bbox `_fit_points` for preview conditioning; adding a second conflicting geometry reference image.

### 221. Level-3 preview: silhouette mask conditioning + deterministic footprint crop (2026-04-13)
- Status: Accepted
- Why: Founder reported §220 still produced worse previews: the model needs an unambiguous **region to fill** (exact walkable silhouette) and the product should **discard** everything outside that region instead of relying on Gemini to frame the room correctly in a fixed canvas.
- Consequence: `scripts/room_environment_system.py` — `_render_room_layout_conditioning_image` is now a **two-tone mask** (near-black void outside the polygon, bright fill inside; faint platform/door tints only). `_generate_level3_image_with_gemini` applies `_crop_level3_gemini_output_to_footprint`: axis-aligned crop from polygon bbox in guide space (`1344×768`, `pad=84`), scaled proportionally when the returned image size differs. Optional `MV_LEVEL3_PREVIEW_CROP_MARGIN` (default 8px, clamped 0–64). Prompt copy describes the mask, void margin, and post-crop. **Rejected:** continuing a full-canvas scenic “layout sketch” as reference #1 without crop; relying on the model to omit exterior pixels without a mask + deterministic crop.

### 222. Level-3 preview: polygon alpha punch after crop (concave / R1) (2026-04-13)
- Status: Accepted
- Why: Founder reported preview **shape** still wrong for **R1** despite §221. Root cause: **axis-aligned crop only** yields a **rectangle** the size of the polygon’s bounding box; for **L-shaped / concave** footprints, large regions are **inside the bbox but outside the walkable polygon** (notches). Gemini and the mask guide still leave misleading pixels there unless we enforce the silhouette in post.
- Consequence: `scripts/room_environment_system.py` — `_postprocess_level3_gemini_output` (used by `_crop_level3_gemini_output_to_footprint`): after crop, map `_geometry_footprint_polygon_output_pixels` from guide space into cropped image pixels (respecting Gemini output scale vs `1344×768`) and **multiply alpha** with a filled polygon mask — outside = **transparent**. Opt-out: `MV_LEVEL3_PREVIEW_POLYGON_MASK=0`. Prompt notes concave punch-out. **Rejected:** relying on bbox crop alone for non-convex rooms; heuristic flood-fill from seed (fragile vs holes).

### 223. Level-3 + background_far_plate: forbid mask-tracing “shell” rim (2026-04-13)
- Status: Accepted
- Why: Founder screenshot showed **thick stone following the stepped preview outline** — reads as **`room_shell_foreground` duplicated onto the background / preview plate**. Gemini treats the bright mask edge like a portal frame unless explicitly forbidden; prompts alone were insufficient for a crisp fix.
- Consequence: `scripts/room_environment_system.py` — (1) **Level-3** prompt + footprint contract: no masonry ring / tunnel mouth tracing the mask; interior inset from boundary; `room_shell_foreground` owns perimeter stone. (2) **`background_far_plate`** `component_rules` + silhouette clause: no stone lip hugging the void outline; first wall mass inset from crop line. (3) **Post:** optional **inward mask erosion** before alpha multiply — default `MV_LEVEL3_PREVIEW_MASK_ERODE_PX=2` (3×3 min filter passes, clamp0–12); `0` disables; if erosion would zero the mask, it **reverts** to the un-eroded mask. **Rejected:** accepting mask-edge shell as “background”; huge default erosion without revert guard.

### 224. Runtime review: headless capture reliability + composite fallback not always “fail” (2026-04-13)
- Status: Accepted
- Why: Founder saw **blank runtime review** and **`composite_fallback · headless_browser_failed`** with preview tiles **Failed**. Causes: (1) `capture_runtime_review.js` assumed **global `WebSocket`** (missing on Node &lt;21 unless `ws` is installed); (2) fixed CDP port **9236** could collide; (3) `try`/`except`/`else` on browser capture **overwrote** `headless_browser_invalid_frame` with a generic `headless_browser_failed`; (4) **`browser_capture_required` was always a hard fail** even when the **PIL composite** produced a **usable** PNG; (5) subprocess stderr was discarded.
- Consequence: `scripts/capture_runtime_review.js` — WebSocketCtor with **`ws` fallback** + short startup delay before `/json/list`. `scripts/room_environment_system.py` — `_pick_free_tcp_port()` (bind0, **OSError fallback** pid-scoped port for sandboxed CI); pass **`--port`** + **60s** script timeout; **120s** subprocess timeout; **log stderr** on non-zero exit; **remove erroneous try/else** overwrite; `_run_runtime_review` — if capture is not headless but **`_runtime_review_capture_is_usable`**, **`browser_capture_degraded` warning** only (not `browser_capture_required`). `room-layout-editor.html` — tip also when degraded. Tests: subprocess mock `returncode=0`; new degraded-path test. **Rejected:** silently accepting blank composite as pass; requiring Node `ws` without documenting Node21+ alternative.
- Follow-up (2026-04-13): **CDP `Page.captureScreenshot` captures the top document only.** `runtime-capture.html` is mostly `#000` with the game in a nested iframe, so headless output stayed **near-uniform dark** and tripped **`headless_browser_invalid_frame`** even when the iframe game was fine. **Fix:** still write `runtime-capture.html` for manual QA, but **`_write_runtime_review_capture_page` returns the game URL** (`/index.html#preview=embed&capture=runtime-review&layout_url=...&start=...`) for the capture helper; `capture_runtime_review.js` prefers a target whose URL matches **`index.html`**, and evaluates boot/canvas on **`window` / `document` when `#preview` is absent** (direct game load). **Rejected:** lowering `_runtime_review_capture_is_usable` thresholds to accept wrapper-sized black frames.

### 225. Cursor Cloud Agents PNG path — reverted (2026-04-13)
- Status: Rolled back (founder decision)
- Why: Experimental **Cursor Cloud Agents** integration for **level-3 previews** and **bespoke image** generation (vs Gemini) was removed from the product surface; agents are slow, repo-scoped, and not the desired long-term image path.
- Consequence: **Removed** `scripts/cursor_cloud_agent_client.py`, UI provider toggle, request `ai_provider`, and `room_layout_copilot` `cursorCloudAgents` ping fields tied to room-env PNGs; **`.env.local.example`** `CURSOR_*` / `MV_ROOM_ENV_IMAGE_PROVIDER` lines for that feature. Room environment images are **Gemini-only** again in this repo. **Unchanged:** Agent OS **issue chat** may still use `CURSOR_API_KEY` / Cloud Agents in `scripts/os_dashboard_supervisor.py` — that is separate from room wizard PNG generation.
- **Rejected (for room env):** treating Cloud Agents as a drop-in replacement for Gemini image calls; maintaining dual PNG backends without founder pull-through.

### 226. `background_far_plate`: ban duplicate “portal / picture-frame” shell (2026-04-13)
- Status: Accepted
- Why: Founder screenshot showed the **background** layer still reading as a **second chamber shell** — a near stone frame with a rectangular “view through” opening (proscenium / portal), plus unmotivated **black corner wedges**, while `room_shell_foreground` already owns the rim. Stale **design_intent** and defaults that said “inside the opening only” reinforced portal language.
- Consequence: **`scripts/room_environment_system.py`** — expanded `background_far_plate` `component_rules`, silhouette clause, and `prompt_opening` to forbid portal/proscenium/near-wall aperture compositions, bottom stair skirts as outer frames, and matte-black corner gutters; default **`enclosure_architecture`** and **`_default_component_prompts` background** text de-emphasize border/silhouette framing; **`_normalize_single_component_schema` (background)** resets **`design_intent`** to fallback when toxic tokens (`looking through`, `portal frame`, etc.) appear; extended stale token list. **`room-wizard-environment.js`** default `enclosure_architecture` aligned with Python. Tests: `test_normalize_background_schema_resets_portal_design_intent`, updated stale-shell test. **Rejected:** relying on §223 mask erosion alone without prompt/schema hygiene; keeping “inside the opening only” wording for background defaults.

### 227. `background_far_plate`: drop footprint silhouette from Gemini reference stack (2026-04-13)
- Status: Accepted
- Why: Prompt/schema tightening in §226 was not enough — **`_write_bespoke_room_silhouette_reference`** for **`background_far_plate`** (black band = shell-adjacent occupancy) **strongly invites** the model to paint another heavy stone rim, which stacks on **`room_shell_foreground`** regardless of text saying not to.
- Consequence: **`_bespoke_reference_images_for_component`** for **`background_far_plate`** returns **`[depth-tone guide only]`** — **`_structural_slot_reference_guide`**, no silhouette PNG, no template in the reference list. **`_build_bespoke_prompt`** footprint block states geometry is **text + chamber bounds**; reference #1 is the flat schematic guide only. **`midground_side_frame`** and structural slots **unchanged** (still use silhouette when polygon valid). **Tradeoff:** footprint fidelity for the far plate leans more on **text** + existing geometry hints than on a raster silhouette. **Rejected:** keeping the silhouette “for alignment” while unified shell is on; assuming prompt text alone can override a shell-shaped conditioning map.

### 228. Founder baseline: R1 scenic stack without duplicate background shell (2026-04-14)
- Status: Accepted (baseline lock)
- Why: Founder confirmed workbench + runtime review for QA project **`room-ai-helpfulness-qa-67562113`**, room **R1**, after **`background_far_plate` guide-only** refs (§227), server restart, and regeneration — **no acceptable second rim** on the far plate over **`room_shell_foreground`**.
- Consequence: **`docs/baselines/room-environment-r1-scenic-shell-2026-04-14.md`** describes the lock; **`tests/fixtures/room_environment_r1_scenic_baseline.json`** records project/room ids, decisions **214–217, 226–228**, **`guide_only`** contract flag, and canonical artifact paths (under `projects-data`, often gitignored). **`tests/room_environment_r1_scenic_baseline.test.py`** validates the fixture in CI; **`.github/workflows/test.yml`** runs it. Optional git tag **`baseline/room-environment-r1-scenic-2026-04-14`**. **Rejected:** treating regenerated PNGs as committed golden files in-repo (too heavy / gitignored); re-adding **`background_far_plate`** silhouette refs without updating this baseline.

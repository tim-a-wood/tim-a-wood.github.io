...
- Status: Accepted (2026-04-11)
- Why: §200 aligned the **image** outer bbox to the chamber AABB, but founder evidence still showed the **walkable hole** shifted (stone too far up-left; ~half a pier width in X/Y). Raster analysis of `R1-room-shell.png`: **alpha0** pixels’ centroid sits **left and down** of the image center (~31px / ~8px at1080×960) because the opaque pier mass is **asymmetric** (heavy right column, stepped L-void). Anchoring the image center on the chamber center therefore leaves the **opening** off-center vs the footprint and vs scenic layers masked to the polygon.
- Consequence: After bbox+origin placement, runtime applies a **one-time** correction from `getUnifiedShellOpeningCentroidTexture` (canvas `getImageData`, alpha===0 centroid, cached per texture): shift the sprite by `-(oc - imageCenter_tex) × (display/native)` so the **opening centroid** targets the **chamber bbox center** (same target the image center had before). Rejected: hard-coding a single pixel offset per QA room; using low-alpha thresholds that blend semi-opaque fringe into the “opening” and bias the centroid.

...
function defaultEnvironmentComponentSchemas(description = '', components = {}) {
  ...
  return {
    ...
    background: {
      design_intent: legacyPrompt('background') || `${base} Background must read as far-depth interior space only, subordinate to a separate shell layer, with a calm open center.`,
      visual_role: 'far_depth',
      material_family: 'far-depth architectural stone interior',
      silhouette_rules: ['interior depth only', 'open center lane'],
      detail_density: 'medium',

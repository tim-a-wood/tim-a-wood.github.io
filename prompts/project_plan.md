...
- **Room layout editor (`room-layout-editor.html`):** Shell markup + inline seed JSON; **CSS** split under `css/` (`editor-tokens.css`, `editor-shell.css`, `room-wizard.css`). **Wizard helpers** live in `js/wizard/` (six modules). **Editor logic** loads from `js/editor/preamble.js` plus chunked sources (`chunk-0.js`–`chunk-4.js`) assembled by `load-chunks.js` (lift-and-shift from the former inline IIFE; `RoomEditor.State` / `RoomEditor.Ui.refs` namespace). Regenerate chunks after editing the editor source via `python3 scripts/generate-room-editor-chunks.py` once the monolith is reintroduced or a new source-of-truth file exists. Full 13-file UMD map per `/.claude/plans/fizzy-beaming-cray-spec.md` is deferred. World/Room workflow toggle; **Layout** (footprint, neighbors, platforms); **Environment** (RW-4 + RW-4b Copilot); **Review**. See `docs/room-wizard-implementation-sprints.md`.
- **Room editor — product intent:** The **editor + wizard are the product** for **solo metroidvania developers** (including entry-level experience). Prefer **defaults**, **plain-language UI**, and **guided flows**; hide schema, graph internals, and engine jargon unless the user opens advanced/export paths. Optional **AI-assisted authoring** (Environment Copilot, RW-4b) should **propose** structured data the user **approves** before it touches the layout — no silent mutation.
...
- **Tech:** HTML/JS, Phaser 3 (Arcade Physics), texture generation at runtime. No external art assets. `manifest.json` is present; service-worker caching is intentionally disabled during rapid iteration on GitHub Pages to avoid stale installs. The **game** reads `room.environment` for **visual hooks** (e.g. camera background + parallax tint) — see `index.html` (`ROOM_ENV_THEME_BG`, `ROOM_ENV_STARFIELD_TINT`); generation stays **code-driven**, not per-frame AI.

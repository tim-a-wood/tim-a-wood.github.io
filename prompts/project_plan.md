...
- **Room layout editor (`room-layout-editor.html`):** World/Room workflow toggle; **Layout** (footprint, neighbors, platforms); **Environment** phase (RW-4: `room.environment` theme + tags; **RW-4b Copilot**: Gemini via `scripts/sprite_workbench_server.py` + `.env.local`, apply-before-save in UI); **Review**. Arbitrary polygon floors deferred. See `docs/room-wizard-implementation-sprints.md`.

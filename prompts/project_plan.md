...
- **Room layout editor (`room-layout-editor.html`):** Shell markup; default layout seed in `room-layout-seed.json` (loaded by `js/editor/init.js`). **CSS** under `css/` (`editor-tokens.css`, `editor-shell.css`, `room-wizard.css`). **Wizard helpers** in `js/wizard/`. **Editor logic** is layered UMD under `js/editor/` (`constants`, `state`, `model`, … `init.js`), produced from `scripts/_merged_editor_body.js` by `node scripts/build-room-editor-modules.cjs` (edit the merge file, then rebuild). Legacy `chunk-*.js` + `load-chunks.js` and `python3 scripts/generate-room-editor-chunks.py` are superseded for shipping. World/Room workflow toggle; **Layout** (footprint, neighbors, platforms); **Environment** (RW-4 + RW-4b Copilot); **Review**. See `docs/room-wizard-implementation-sprints.md`.

...
- [2026-04-07] **My Actions file references.** When adding or updating a `founder_decisions` entry or a `needs-review` priority in `finance-status.json`, include the specific file paths the founder needs to open — wrap any `artifacts/` path in backticks (e.g., `` `artifacts/finance/paid-tier-margin-template.md` ``) so the My Actions dashboard extracts it as a clickable link. Paths under `docs/`, `decisions/`, or `research/` auto-extract without backticks. Never write "see the research library" or "see the artifact" without naming the exact file path. Trigger: every time a `founder_decisions` or `needs-review` entry is written or updated. Context: Founder directive — My Actions detail panel must show directly relevant files, not just the status JSON and charter.

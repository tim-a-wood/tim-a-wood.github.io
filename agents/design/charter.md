...
- [2026-04-03] **High-fidelity mockup before UI implementation.** Whenever the founder gives you a task to update a UI or any front-end component, you must first create a high-fidelity mockup of the intended result before making implementation changes. The mockup should be visually authentic and close to a real final product, but it does not need to be functional. After creating the HI-FI mockup, use it as the exact visual reference for the implementation. Then implement the real UI so that it matches the approved mockup with no visual change or drift. Do not skip straight to implementation for UI tasks. Do not redesign during implementation. Implementation must be a faithful translation of the HI-FI mockup into the actual front end. Trigger: any UI or front-end change request. Context: Founder directive—Design owns visual fidelity and prevents implementation drift.

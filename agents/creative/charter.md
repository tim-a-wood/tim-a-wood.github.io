...
- [2026-04-04] **Pixel art quality standards — mandatory with art bible.** For all game-facing pixel production (sprites, sheets, and briefs that commit art to the build), treat [`docs/pixel-art-quality-standards.md`](../../docs/pixel-art-quality-standards.md) as **mandatory alongside** the Ashen Hollow art bible: the bible holds direction and `AH-*` contract; the standards doc holds execution gates (palette discipline, silhouette, anti-aliasing, AI cleanup, §5 checklist). Instruct contributors and agent sessions to use **both**; if a brief or bible change would conflict with the standards, resolve explicitly (update standards with Creative + Animation alignment, or adjust bible scope) before production proceeds. Trigger: art direction, production briefs, concept-to-sprite sign-off, or creative review of in-game raster art. Context: Founder directive — enforce consistent agentic use of published pixel standards during game development.

...
- [2026-04-04] **My Actions alignment after every request.** After completing any request, review whether `orchestration-status.json` (and any other status files you changed) need updates so the Agent OS **My Actions** view stays truthful: `founder_decisions`, `priorities` in `needs-review`, and related fields per `agents/design/dashboard-standard.md`. Update when needed; if nothing founder-facing changed, performing the check without edits is sufficient. **All specialists:** same rule for their own `*-status.json` — see `AGENTS.md` section *My Actions dashboard — check after completing every request*. Trigger: end of every completed request. Context: Founder directive — founder task board must mirror reality.

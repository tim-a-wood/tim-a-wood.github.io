...
- Pixel art quality standards ([`docs/pixel-art-quality-standards.md`](../../docs/pixel-art-quality-standards.md) — production gate with art bible), AI animation workflow recommendations → Animation (+ Creative for direction; Game Director for milestone framing; Engineering for pipeline)
...
- [2026-04-04] **Pixel art quality standards — routing and synthesis.** When routing or synthesizing work that involves **production in-game sprites or pixel art**, explicitly anchor briefs and handoffs to [`docs/pixel-art-quality-standards.md`](../../docs/pixel-art-quality-standards.md) and the art bible; assign or name-check Animation (audit/spec), Creative (direction), Level Design (readability in rooms), Game Director (milestones), and Engineering (pipeline) as the task requires. Do not summarize sprite delivery as complete in digests or escalations without noting gate status against that document. Trigger: multi-agent sessions, digest lines, or task decomposition touching build-bound raster art. Context: Founder directive — consistent agentic use of pixel standards during game development.

- [2026-04-04] **My Actions alignment after every request.** After completing any request, review **your** `*-status.json` so **My Actions** stays truthful. You **may add, edit, or remove** founder-facing rows **you own** in that file (`founder_decisions`, `priorities` in `needs-review`) when the board should reflect new, resolved, or corrected work — including **removing** obsolete cards. Do not change another agent’s status JSON for My Actions unless the founder explicitly assigned that. Follow `agents/design/dashboard-standard.md`. If nothing founder-facing changed for your agent, check without edits is enough. **All specialists:** see `AGENTS.md` *My Actions dashboard — check after completing every request*. Trigger: end of every completed request. Context: Founder directive — founder task board must mirror reality.

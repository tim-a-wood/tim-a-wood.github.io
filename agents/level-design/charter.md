...
- [2026-04-04] **Pixel art quality standards — mandatory reference.** When game development tasks touch **in-game pixel readability** (room/environment visual spec, Copilot or prompt text that steers tile or sprite language, placement notes that assume silhouette or palette at game scale, or handoffs that imply production art), **read and apply** [`docs/pixel-art-quality-standards.md`](../../docs/pixel-art-quality-standards.md) together with the art bible so spatial design and art execution stay aligned. Flag proposals that would force sprites or environment art to violate that gate. Trigger: level-design specs, room/environment quality reviews, or Copilot prompt changes that affect how art is described or validated for the build. Context: Founder directive — enforce consistent agentic use of published pixel standards during game development.

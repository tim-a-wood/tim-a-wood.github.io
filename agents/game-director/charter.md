...
- [2026-04-04] **Pixel art quality standards — product gate.** When defining milestones, acceptance, or sign-off for **game-facing pixel art** (sprites, sheets, or build-bound raster), frame requirements using [`docs/pixel-art-quality-standards.md`](../../docs/pixel-art-quality-standards.md) **together with** the Ashen Hollow art bible. Do not treat sprite or animation milestones as met on narrative or bible tone alone — execution must pass the published checklist (Animation/Creative own verification). Escalate gaps between bible and standards to Creative + Animation before locking content. Trigger: content planning, milestone reviews, digest lines, or greenlight language that references production in-game art. Context: Founder directive — consistent agentic use of pixel standards during game development.

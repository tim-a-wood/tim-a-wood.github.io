...
from scripts.workbench_local_control import PERSONAL_USAGE_CACHE_REL  # noqa: E402
...
def _load_repo_env_files() -> None:
    ...
    for path in (APP_ROOT / "agent_os.env", WORKSPACE_ROOT / ".env.local"):
        if not path.is_file():
            continue
        try:
...
def main() -> int:
    ...
    ap.add_argument(
        "--output",
        type=Path,
        default=WORKSPACE_ROOT / PERSONAL_USAGE_CACHE_REL,

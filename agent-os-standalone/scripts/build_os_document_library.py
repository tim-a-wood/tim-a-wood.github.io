...
from scripts.agent_os_roots import APP_ROOT, WORKSPACE_ROOT

REPO = WORKSPACE_ROOT
MANIFEST_OUT = APP_ROOT / "docs" / "os-documentLibrary.manifest.json"
HTML_OUT = APP_ROOT / "docs" / "os-document-library.html"
DEDUPE_MD_OUT = APP_ROOT / "docs" / "reports" / "os-document-library-dedupe-notes.md"
...
def main() -> int:
    ...
    print(f"Wrote {MANIFEST_OUT.relative_to(APP_ROOT)}")
    print(f"Wrote {HTML_OUT.relative_to(APP_ROOT)}")
    print(f"Wrote {DEDUPE_MD_OUT.relative_to(APP_ROOT)}")
    return 0

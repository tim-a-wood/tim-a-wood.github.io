...
| Unit Test Suite | Overall Unit Tests | General unit test suite status. | Pass | Ran `python3 -m unittest discover -s tests -p "test_*.py"` (84 tests, OK). Change: `_poll_job` returns `last_response`; extractor unwraps job wrapper + `rotationUrls`; tests for job poll + wrapped payload. |

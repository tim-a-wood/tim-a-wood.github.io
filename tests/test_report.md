...
|---|---|---|---|---|
| AT-01 | Fresh Load And Startup | Verify the prototype loads into the playable zone without crashes or blank screens. | Not Run | `index.html` not re-tested; RW-1 work was room editor + unit tests. |
...
| Unit Test Suite | Overall Unit Tests | General unit test suite status. | Pass | Ran `node tests/game-logic.test.js`, `room-editor-export.test.js`, `room-wizard-footprint.test.js` (2026-03-27); all exited 0. |
| RW-1 manual | Room wizard vertical slice | Add Room → Layout → Review → Export JSON (per `docs/room-wizard-implementation-sprints.md`). | Not Run | Automated unit tests only; browser demo not executed in CI. 2026-03-27: wizard inline in Room tab (not modal); Global map vs Room toggled in header; Room setup button; not manually verified in browser. |

...
| Unit Test Suite | Overall Unit Tests | General unit test suite status. | Pass | Ran `python3 -m unittest discover -s tests -p "test_*.py"` (89 tests, OK). Change: Character/Animations panels offer one-click Switch to Pixel Lab when brief is still comfyui (HTML-only). |

...
| Unit Test Suite | Overall Unit Tests | General unit test suite status. | Pass | Ran `game-logic`, `room-editor-export`, `room-wizard-footprint`, `room-wizard-neighbor-align` (2026-03-27); all exited 0; neighbor module includes `computeMatchWallLengthPatch` for axis-aligned rectangle rooms + tests. UI: world workflow rail stays cool dark+cyan; room sub-rail + room dock use inverse light theme; primary steps use distinct accents (cyan / orange / violet) (CSS only, not re-run in CI). |

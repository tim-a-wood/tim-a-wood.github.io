...
| UT-01 | Unit Test Suite | Verifies the full automated unit test suite in `tests/game-logic.test.js` passes. | Not Run | Relic moved to first platform (368,266); platform near key ledge moved left (1100) so key requires double jump; `node` was not available in the current execution environment. |

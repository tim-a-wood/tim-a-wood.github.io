...
| Unit Test Suite | Overall Unit Tests | General unit test suite status. | Pass | Ran `python3 -m unittest discover -s tests -p "test_*.py"` (89 tests, OK). Change: JS helpers hoisted before wizard summary; POST bodies always read; animation frame decode uses canvas `rgba_size`; friendlier animation error toasts. |

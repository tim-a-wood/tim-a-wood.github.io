...
|---|---|---|---|---|
| AT-01 | Fresh Load And Startup | Verifies the game loads into the playable scene with the player, HUD, and controls visible. | Not Run | R2→R3 branch A entry was reworked to use a ceiling door in R2 and a floor door landmark in R3; no manual startup run was executed here. |
| AT-02 | Movement, Camera, And World Bounds | Verifies horizontal movement works and the player/camera stay inside the bounded world. | Not Run | World bounds and camera logic were unchanged; only R2/R3 branch door placement and transition behavior were updated, and movement was not manually re-tested. |
...
| AT-09 | Key Pickup And Door Unlock | Verifies the key can be collected on the high ledge, appears in the HUD inventory, and is consumed when unlocking the left-side door. | Not Run | Key and ability reward logic were unchanged; new R2 ceiling door and R3 floor door visuals for Branch A were not exercised end to end here. |
| UT-01 | Unit Test Suite | Verifies the full automated unit test suite in `tests/game-logic.test.js` passes. | Not Run | `node` was not available in the current execution environment, so the suite could not be run after the updated R2→R3 transition work. |

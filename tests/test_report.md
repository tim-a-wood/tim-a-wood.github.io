...
| Unit Test Suite | Overall Unit Tests | General unit test suite status. | Pass | 2026-03-30: Analytics dashboard layout aligned with Engineering (vertical sections, shared chart grid + `paintOsChartsForView('analytics')`). Priorities/loading fixes unchanged. Smoke: `build_*_dashboard_payload`. Full `pytest tests/` not re-run (HTML/JS-only). Game ATs not run. |

...
| Unit Test Suite | Overall Unit Tests | General unit test suite status. | Pass | 2026-03-29: `python3 -m unittest tests.test_sprite_workbench.SpriteWorkbenchTests.test_usage_ledger_charts_rollup tests.test_sprite_workbench.SpriteWorkbenchTests.test_workbench_settings_and_usage_ledger_round_trip` — Pass. Agent OS: `/api/dashboard-data` now includes `usage_charts` + `usage_summary` (ledger rollups); Analytics dashboard aligned with Eng/Design layout; `docs/analytics-dashboard-kpis.md` added. AT-01–AT-09 (game) not run this session. |

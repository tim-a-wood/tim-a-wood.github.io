...
| Unit Test Suite | Overall Unit Tests | General unit test suite status. | Pass | 2026-03-29: Ledger rollup unit tests previously run (see history). Follow-up: `os-dashboard.html` explains empty charts when `/api/dashboard-data` is missing (wrong origin) vs empty ledger; `canControl` treats workbench server as connected. Browser AT for Agent OS not in acceptance_tests.md. AT-01–AT-09 (game) not run this session. |

...
| RW-1 manual | Room wizard vertical slice | Add Room → Layout → Review → Export JSON (per `docs/room-wizard-implementation-sprints.md`). | Not Run | Automated unit tests only; browser demo not executed in CI. 2026-03-27: World map first pill on workflow rail (glyph, not “step 0”); header Room/Global tabs removed; not manually verified in browser. |

...
| Unit Test Suite | Overall Unit Tests | General unit test suite status. | Pass | Ran `python3 -m unittest discover -s tests -p "test_*.py"` (96 tests, OK). Change: Poll all `background_job_ids` from `/v2/characters/animations` and merge `per_job_last_response` for template animations; frame decode walks merged jobs. |

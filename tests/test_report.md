...
| Unit Test Suite | Overall Unit Tests | General unit test suite status. | Pass | Ran `game-logic`, `room-editor-export`, `room-wizard-footprint`, `room-wizard-neighbor-align`, `room-wizard-terrain` (2026-03-27); all exited 0. Room wizard: full-height flex layout + edge-to-edge dock in canvas panel (CSS only). |

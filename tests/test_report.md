...
| Unit Test Suite | Overall Unit Tests | General unit test suite status. | Pass | Ran `game-logic`, `room-editor-export`, `room-wizard-footprint`, `room-wizard-neighbor-align` (2026-03-27); all exited 0; Align uses perpendicular-only translation so Match opening height can slide tangentially; align+hatch integration test for vertical edges. |

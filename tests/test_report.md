...
| AT-02 | Movement, Camera, And World Bounds | Verifies horizontal movement works and the player/camera stay inside the bounded world. | Not Run | World bounds were unchanged; the room-transition camera behavior was adjusted to use a full black overlay during movement, but this was not manually re-verified. |
...
| AT-09 | Key Pickup And Door Unlock | Verifies the key can be collected on the high ledge, appears in the HUD inventory, and is consumed when unlocking the left-side door. | Not Run | Key and ability reward logic were unchanged; only Branch A’s R2↔R3 door placement on the R2 top ledge, the R3→R2 return spawn height, and the room-transition camera overlay were adjusted. |
| UT-01 | Unit Test Suite | Verifies the full automated unit test suite in `tests/game-logic.test.js` passes. | Not Run | `node` is not available in the current execution environment, so the test suite could not be re-run after the latest Branch A R2↔R3 door placement, R3→R2 spawn, and transition camera overlay changes. |

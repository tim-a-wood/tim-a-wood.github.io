...
| RW-1 manual | Room wizard vertical slice | Add Room → Layout → Review → Export JSON (per `docs/room-wizard-implementation-sprints.md`). | Not Run | Automated unit tests only; browser demo not executed in CI. 2026-03-27: wizard UI refactored to Sprite Workbench scaffold (`phase-rail` / `phase-pill` / `stage-card`); not manually verified in browser. |

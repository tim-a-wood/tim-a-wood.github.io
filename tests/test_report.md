...
| AT-02 | Movement, Camera, And World Bounds | Verifies horizontal movement works and the player/camera stay inside the bounded world. | Not Run | The left-side environment now includes a longer left-edge corridor ledge with the door bar near its right edge; traversal and bounds were not manually re-tested here. |
...
| UT-01 | Unit Test Suite | Verifies the full automated unit test suite in `tests/game-logic.test.js` passes. | Not Run | Updated the automated tests for the longer left corridor and repositioned door, but `node` was not available in the current execution environment. |

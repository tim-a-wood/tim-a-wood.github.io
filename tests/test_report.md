...
| Unit Test Suite | Overall Unit Tests | General unit test suite status. | Pass | Ran `game-logic`, `room-editor-export`, `room-wizard-footprint`, `room-wizard-neighbor-align` (2026-03-27); all exited 0; neighbor module includes `computeMatchWallLengthPatch` for axis-aligned rectangle rooms + tests. Reverted experimental inverse/wizard palette; room rail uses Workbench-aligned accent strip + chip label (CSS only). |

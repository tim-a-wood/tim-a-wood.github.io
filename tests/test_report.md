...
| Unit Test Suite | Overall Unit Tests | General unit test suite status. | Pass | Ran `python3 -m unittest discover -s tests -p "test_*.py"` (96 tests, OK). Change: `pixellab_client` INFO logs after animation POSTs + ~15s poll heartbeats (`scripts.pixellab_client` logger). |

...
| Unit Test Suite | Overall Unit Tests | General unit test suite status. | Pass | Ran `python3 -m unittest discover -s tests -p "test_*.py"` (85 tests, OK). Change: Pixel Lab direction images use raw bytes + loose base64; `_pixellab_open_image_bytes` clearer errors; per-direction `url` in `images` map; `Accept` on asset fetch. |

...
| Unit Test Suite | Overall Unit Tests | General unit test suite status. | Pass | Ran `python3 -m unittest discover -s tests -p "test_*.py"` (78 tests). Change: sprite tool Character step — inline progress bar + activity dock during Pixel Lab create-character (no new Python tests). |

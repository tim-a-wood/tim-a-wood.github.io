...
| Unit Test Suite | Overall Unit Tests | General unit test suite status. | Pass | 2026-03-29: `/api/dashboard-data` embeds `analytics_status` from repo so Analytics priorities work without a separate `analytics-status.json` fetch; smoke check `build_*_dashboard_payload` locally. Game ATs not run. |

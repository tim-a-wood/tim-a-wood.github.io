...
|---|---|---|---|---|
| AT-01 | Fresh Load And Startup | Verifies the game loads into the playable scene with the player, HUD, and controls visible. | Not Run | The room was rebuilt again into a more cave-like interconnected Hollow Knight-style structure; no manual startup run was executed here. |
| AT-02 | Movement, Camera, And World Bounds | Verifies horizontal movement works and the player/camera stay inside the bounded world. | Not Run | The room shell is unchanged, but the new cave shelf layout was not manually re-tested in this environment. |
...
| AT-09 | Key Pickup And Door Unlock | Verifies the key can be collected on the high ledge, appears in the HUD inventory, and is consumed when unlocking the left-side door. | Not Run | Key and relic positions changed for the new cave layout, but progression was not manually played through here. |
| UT-01 | Unit Test Suite | Verifies the full automated unit test suite in `tests/game-logic.test.js` passes. | Not Run | Tests updated for two-room layout (room 2 with exit in top left); `node` was not available in the current execution environment. |

...
| Unit Test Suite | Overall Unit Tests | General unit test suite status. | Pass | Ran `game-logic`, `room-editor-export`, `room-wizard-footprint`, `room-wizard-neighbor-align`, `room-wizard-terrain`, `room-wizard-environment` (2026-03-27); all exited 0. Game: HUD + `ROOM_ENV_THEME_BG` hooks for `room.environment`; samples in `samples/` + R1–R3 in `room-layout-data.json`. |

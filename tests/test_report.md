...
| Unit Test Suite | Overall Unit Tests | General unit test suite status. | Pass | Ran `python3 -m unittest discover -s tests -p "test_*.py"` (98 tests, OK). Change: parallel round-robin poll for multi `background_job_ids` (fail fast); `test_poll_jobs_parallel_surfaces_failure_without_finishing_first_job`. |

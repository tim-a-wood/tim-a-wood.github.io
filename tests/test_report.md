...
| Unit Test Suite | Overall Unit Tests | General unit test suite status. | Pass | Ran full CI chain (2026-03-27); all exited 0. Canonical layout API (`/api/layout`) shared via `scripts/room_layout_canonical.py` on Sprite Workbench + optional layout-only server. |

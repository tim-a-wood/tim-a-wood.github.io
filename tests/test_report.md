...
| Unit Test Suite | Overall Unit Tests | General unit test suite status. | Pass | Ran `python3 -m unittest discover -s tests -p "test_*.py"` (89 tests, OK). Change: Animations preview falls back to `animation_clips` + default dir with frames; sidebar counts real frame arrays (HTML-only). |

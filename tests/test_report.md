...
| Unit Test Suite | Overall Unit Tests | General unit test suite status. | Pass | Ran `python3 -m unittest discover -s tests -p "test_*.py"` (94 tests, OK). Change: Pixel Lab animation frame extraction (WebP/JPEG/GIF, Base64Image dicts, smaller PNGs, URL fallback); `animate_with_skeleton` polls async jobs; added `PixelLabAnimationFrameExtractionTests`. |

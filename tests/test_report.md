...
| Unit Test Suite | Overall Unit Tests | General unit test suite status. | Pass | Ran `game-logic`, `room-editor-export`, `room-wizard-footprint`, `room-wizard-neighbor-align` (2026-03-27); all exited 0. Room editor: World/Room toggle uses chip styling (`workflow-scope-chip`), `width:auto` so controls centre; matches `.chip` / accent active state. |

...
| Unit Test Suite | Overall Unit Tests | General unit test suite status. | Pass | 2026-03-30: `os-dashboard.html` clears Analytics priorities from embedded API or static JSON (always replaces Loading); fallback hint when fetch fails. Smoke: `build_*_dashboard_payload` returns `analytics_status.priorities`. Full `pytest tests/` not re-run (HTML-only). Game ATs not run. |

...
| Unit Test Suite | Overall Unit Tests | General unit test suite status. | Pass | Ran `python3 -m unittest discover -s tests -p "test_*.py"` (104 tests, OK; network allowed for ComfyUI-dependent test). Change: Pixel Lab `animate_character` uses `_poll_jobs_parallel_until_settled` then serial refill for **failed directions only** (no longer aborts in-flight jobs on first failure); tests `test_poll_jobs_parallel_until_settled_allows_other_jobs_to_finish`, `test_animate_character_serial_refill_only_failed_directions`. |

...
| Unit Test Suite | Overall Unit Tests | General unit test suite status. | Pass | Ran `python3 -m unittest discover -s tests -p "test_*.py"` (108 tests, OK). Change: Phase 7.7 — simplified guided AI wizard (`describe`→`concepts`→`character`→[`animations`]→`export`) in `sprite_workbench_server.py` + `tools/2d-sprite-and-animation/index.html`; legacy full `WIZARD_STEPS` unchanged for non-AI projects. |

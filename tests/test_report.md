...
| Unit Test Suite | Overall Unit Tests | General unit test suite status. | Pass | Ran `python3 -m unittest discover -s tests -p "test_*.py"` (100 tests, OK; network allowed for ComfyUI-dependent test). Change: Pixel Lab `animate_character` serial-per-direction fallback after multi-job poll failure; `PIXELLAB_ANIMATE_SERIAL_FALLBACK` in `.env.local.example`; tests `test_animate_character_falls_back_to_serial_when_parallel_fails`, `test_animate_character_serial_by_direction_merges_in_order`. |

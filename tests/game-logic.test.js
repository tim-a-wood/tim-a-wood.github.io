...
(function testIndexUnifiedShellUsesPremaskedTexture() {
    ...
    assert.ok(
        html.includes('addRoomBespokeUnifiedShellForegroundDecor(roomId, shellSupport)')
            ...
            && html.includes('computeUnifiedShellWorldPlacement(roomId, asset, support)')
            && html.includes('localX: curLeft, localY: curTop')
            && html.includes('setRoundPixels(true)')
            && html.includes('SHELL_TEXTURE_PREMASK')
            && html.includes('hashRoomFootprintForShellPremask')
